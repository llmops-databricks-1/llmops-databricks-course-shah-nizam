...
class VectorSearchManager:
    ...
    def __init__(
        ...
        self.source_table = f"{self.catalog}.{self.schema}.aon_chunks_table"
        self.index_name = f"{self.source_table}_index"
    ...
    def create_or_get_index(self):  # noqa: ANN201
    ...
    def sync_index(self, timeout_minutes: int = 15) -> None:
        ...
        while time.time() < deadline:
            ...
            logger.info(f"Index not ready (state: {detailed_state}). Waiting 30s...")
            time.sleep(30)
        else:
            raise TimeoutError(
                f"Index {self.index_name} not ready after {timeout_minutes} minutes"
            )
